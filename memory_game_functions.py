...
def make_cards(cards_num: int, deck_name: list) -> None:
    ...
    class gamestats attribute

    Parameters:
        card_num: the number of cards the player picked to play with
...
def make_deck(final_cards, num_of_cards, card_x, card_y):
    ...
    Parameters:
        card_num: the number of cards the player picked to play with
        final_cards: card pairs stored in random indices
        GAME_STATS.cards_created: a gamestats attribute used to store the instances of Cards
...
def find_pair(x_cord: int, y_cord: int) -> None:
    ...
    Parameters:
        x_ord: x coordinate for player's click on screen
